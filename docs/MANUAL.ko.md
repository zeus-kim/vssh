...
vssh memory discover                                # 플릿 전체 자동 탐지(계획)
vssh memory discover --apply                        # …그리고 기록
```

**discover가 핵심**: 알려진 모든 피어를 병렬로 프로브해서, 각 노드가 실제로
*무엇을 돌리는지*(GPU `nvidia-smi`·모델→`rtx4090` 태그, 실행 중인 유닛, 리스닝
포트, 컨테이너, 디스크)로부터 그 노드가 *무엇인지*를 추론한다 — 인벤토리를 손으로
관리할 필요가 없다. 기본은 계획만, `--apply`로만 기록하며 **노트는 보존**되고
`role`/`services`/`tags`만 재도출된다. role 판정: `gpu`(GPU 있음) → `mail`(메일
데몬 또는 :25/:587/:993 중 2개 이상 — 로컬 MTA가 :25만 열어둔 평범한 노드를
오분류하지 않음) → `storage`(≥2TB) → `network`(프록시) → `vm`.
아래 `@role`/`@tag`/`@service` 셀렉터가 플릿과 어긋나지 않게 유지해주는 것이 바로
이것이며, `scripts/refresh_fleet_state.sh`가 스케줄러에서 재실행한다(6–12시간마다,
`DISCOVER=0`으로 해제). 노드 역할이 바뀌어도 **손 안 대고** 셀렉터가 최신을 유지.
