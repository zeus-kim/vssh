...
> **SSH는 사람을 위해 설계되었습니다. vssh는 AI 에이전트를 위해 설계되었습니다.**

**AI 실행 런타임** — 구조화·범위제한·감사 원격 실행. `sshd` 없이.

<!-- TODO: 데모 GIF 추가: vssh run g1 "nvidia-smi" → 구조화된 JSON 증거 -->
...
**MCP 서버 내장.** 바이너리 하나. 추가 프로세스 없음. JSON 편집 없음.

```bash
vssh mcp-install --client claude   # 또는: cursor, codex, gemini
```
...
| 기능 | 설명 |
|------|------|
| **Fleet Memory** | AI가 각 노드의 역할·서비스·히스토리를 기억 |
| **Intent** | "웹 서버들 디스크 확인" → 검증된 명령 계획 |
| **Workflow** | 사전 정의 다단계 작업, 이름으로 호출 |
| **Diff** | 감사 로그 변경 요약 |
| **Evidence** | 타입 결과 (stdout/stderr/exit/duration), 텍스트 아님 |
| **Audit** | 해시체인, 키 귀속, 변조 방지 |
...
```bash
# 원라인 (Linux x86-64/arm64/arm/386/riscv64/ppc64le/s390x, macOS)
...
`vssh mcp`는 파싱할 텍스트가 아닌 구조화된 실행 증거를 반환합니다 — vssh를 단순한
SSH 대체가 아닌 **AI 실행 런타임**으로 만드는 핵심입니다.

### AI 네이티브 기능

| 카테고리 | 도구 | 가능하게 하는 것 |
|----------|------|-----------------|
| **Memory** | `vssh_memory` (get/set/note/auto_note/find/ask) | 노드별 역할·서비스·태그·노트 — AI가 플릿을 기억합니다. |
| **Intent** | `vssh_intent` | 자연어 → 명령 계획. "웹 서버들 디스크 확인해줘" → 정책 검증된 실행 계획. |
| **Workflow** | `vssh_workflow` (list/run/status) | AI가 이름으로 호출할 수 있는 사전 정의 다단계 흐름. |
| **Diff** | `vssh_diff` | 감사 로그 변경 요약 — 뭐가, 언제, 누가. |

### 핵심 실행 도구

| 그룹 | 도구 |
|------|-------|
| 탐지 / 상태 | `vssh_fleet` (doctor, status, list, hosts, state, setup) |
| 라우팅 | `vssh_route` (select, policy_check) |
| 실행 | `vssh_exec` (plain, safe, routed, many) |
| 타입드 RPC / facts | `vssh_query` (facts, facts_many, rpc, rpc_many) |
| 잡 / 아티팩트 | `vssh_job` (start, status, logs, cancel), `vssh_transport` (tunnel, artifact_collect) |
| 설정 (게이트) | `vssh_config` (list, authorize_key, revoke_key, set_node, pin_node) |

**안전 모델:** 파괴적 명령(`rm -rf`, `shutdown`, `reboot`, `docker rm`,
`kubectl delete` …)은 명시적 사람 승인 후 `allow_dangerous: true`가 아니면 **차단**.
설정 변경 도구는 `VSSH_ALLOW_CONFIG_WRITE=1` 필요. 모든 응답은 구조화된 증거 봉투.
[docs/MANUAL.ko.md](docs/MANUAL.ko.md) 참고.
...
- [AI 런타임](docs/AI_RUNTIME.md) — Memory, Intent, Workflow, Diff 기능
...
- [예제](examples/) · [SECURITY.md](SECURITY.md) · [CONTRIBUTING.md](CONTRIBUTING.md) · [CHANGELOG.md](CHANGELOG.md) · [English README](README.md)
